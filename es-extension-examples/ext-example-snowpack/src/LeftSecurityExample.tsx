...
import { publishJson, subscribeJson, unsubscribe } from '@gots/noowf-inter-widget-communication';
...
export const LeftSecurityExample: React.FC<{ es: ExtensionScaffoldApi }> = ({ es }) => {
    ...
    React.useEffect(() => {
        // request the princiapl info from pubsub right now
    ...
    const getPrincipalEntry = (label: string, value: any) => {
      ...
      const getPrincipalContent = (principal: any) => {
        const { userId, citizenship, clearance, source, opauth } = principal;
        const css = `
          .security-pair {
            position:relative;
            left:0;
