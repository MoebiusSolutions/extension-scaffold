...
The example below adds a rotating icon to the *Example* tab:
...
```tsx
const DocumentControlsGroupOne = () => {
  return <es-ribbon-section label="Group 1">
...
```tsx
  const Example = () => ( <es-ribbon-button label="New" onClick={showNew}>
      <NewIcon />
  </es-ribbon-button> )
